    ...
    async def initialize_app(self, app: Kui):
        ...
        try:
            ...
            if "list_audio_backends" in str(e):
                logger.warning(f"Torchaudio compatibility issue detected: {e}")
                logger.info("Applying compatibility patch...")
                
                # Apply the patch dynamically
                import torchaudio
                import fish_speech.inference_engine.reference_loader as reference_loader
                
                # Patch the __init__ method of ReferenceLoader
                original_init = reference_loader.ReferenceLoader.__init__
                ...
                def patched_init(self):
                    ...
                    try:
                        ...
                        # If list_audio_backends fails, set a default backend
                        self.audio_backends = ['soundfile']
                        logger.info("Using fallback audio backend: soundfile")
                
                reference_loader.ReferenceLoader.__init__ = patched_init
